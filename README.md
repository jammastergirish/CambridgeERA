...
> [!NOTE]
> **Multi-GPU Support (`device_map="auto"`)**: Passing `--device auto` (which is the default in the shell scripts) will automatically trigger HuggingFace Accelerate's `device_map="auto"`. This will seamlessly distribute the model across all available GPUs if you have a multi-GPU setup, preventing CUDA Out-Of-Memory errors during heavy training runs.
