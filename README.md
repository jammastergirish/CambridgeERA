...
> [!NOTE]
> **Multi-GPU Support (`device_map="auto"`)**: Passing `--device auto` (which is the default behavior across the repository) will automatically trigger HuggingFace Accelerate's `device_map="auto"`. This will seamlessly distribute the model across all available GPUs during unlearning runs, experiment evaluations, and inference apps. Use this if you have a multi-GPU setup to prevent CUDA Out-Of-Memory errors on large models.
