...
> GPUS_PER_JOB=4 ./unlearn/parallel_sweep.sh ./unlearn/sweep_unlearn3.sh  # skip probe
> ```
> The wrapper:
> 1. **Probes VRAM** (~30 s) to find the minimum number of GPUs needed to fit the model.
> 2. **Picks group size** — the smallest power-of-two group (1/2/4/8) that fits the model and divides evenly into the total GPU count.
> 3. **Swaps in a shim**: temporarily replaces `run_unlearn.sh` with a lightweight dispatcher that assigns each sweep config to a free GPU group and runs it in the background, then immediately returns so the sweep loop can queue the next config.
> 4. **Waits for all jobs**, then restores the original `run_unlearn.sh`.
>
> Example: 8× A40s (48 GB each), model needs 4 GPUs → **2 sweep configs run simultaneously**, halving total wall-clock time. Falls back to serial execution if only one group fits or the probe fails.
>
