...
> [!NOTE]
> **Multi-GPU Support (automatic)**: `run_unlearn.sh` automatically detects available GPUs and selects the optimal parallelization strategy:
>
> 1. **GPU count detection** — reads available GPUs via `nvidia-smi`.
> 2. **VRAM probe** — loads the model with `device_map="auto"` on a throwaway process to measure how many GPUs it needs and how much memory it uses (~30 s one-time cost).
> 3. **Group size selection** — picks the smallest power-of-two GPU group (1 / 2 / 4 / 8) that fits the model and divides evenly into the total GPU count. For example, with 8× A40s (48 GB each) and a model requiring 4 GPUs to fit: it runs **2 jobs in parallel**, each pinned to 4 GPUs, doubling sweep throughput vs. running serially.
> 4. **Parallel dispatch** — re-invokes itself as worker processes with `CUDA_VISIBLE_DEVICES` set per group. Worker logs go to `/tmp/run_unlearn_group<N>_<PID>.log`.
>
> Falls back gracefully to a single job across all GPUs if the probe fails or only one group fits.
>
> Override with `GPUS_PER_JOB=N` to skip probing and force a specific group size:
> ```bash
> GPUS_PER_JOB=4 ./unlearn/run_unlearn.sh cb_lat
> ```
